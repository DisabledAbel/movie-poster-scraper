...
export function createRedisLatestPosterAdapter({ url, token, key = DEFAULT_KEY, fetchImpl = fetch }) {
  const request = async (command, value, signal) => {
  ...
  return {
    ...
    async set(record, signal) {
      await request("SET", JSON.stringify(record), signal);
    },
  };
}
...
export function createFileLatestPosterAdapter({ file = path.join(getCacheDirectory({}), "latest.json"), fileSystem = fs } = {}) {
  return {
    async get(signal) {
      try {
        return JSON.parse(await fileSystem.readFile(file, { encoding: "utf8", signal }));
      } catch (error) {
    ...
    async set(record, signal) {
      ...
      await fileSystem.writeFile(file, JSON.stringify(record, null, 2), { signal });
    },
  };
}
...
export function createLatestPosterStore({ adapter = createLatestPosterAdapter(), env = process.env, now = () => new Date() } = {}) {
  ...
  return {
    async getLatestPoster() {
      try {
        return await withTimeout((signal) => adapter.get(signal), milliseconds);
      } catch (error) {
    ...
    async saveLatestPoster(title, urlOrObject) {
      ...
      try {
        await withTimeout((signal) => adapter.set(record, signal), milliseconds);
        return record;
      } catch (error) {
        throw new LatestPosterStorageError("Latest-poster storage is unavailable", error);
      }
