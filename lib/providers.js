...
export async function fetchImdbPosterCandidates(movie, year) {
  ...
  // Prioritize exact title matches, but include other matches to ensure we have up to 15 posters
  const exactMatches = filteredItems.filter((item) => item?.l?.toLowerCase() === normalizedSearch);
  const otherMatches = filteredItems.filter((item) => item?.l?.toLowerCase() !== normalizedSearch);
  const prioritized = [...exactMatches, ...otherMatches].slice(0, 15);
...
export async function findPostersSequential(movie, year, options = {}) {
  ...
  let imdbPosters = [];
  try {
    imdbPosters = await fetchImdbPosterCandidates(movie, year);
  ...
  if (imdbPosters.length > 0) {
    const currentPoster = imdbPosters[0];
