...
export async function findPostersSequential(movie, year, options = {}) {
  ...
  // First, get the IMDb poster (the "current" official poster)
  let currentPoster = null;
  try {
    currentPoster = await fetchImdbSinglePoster(movie, year);
  } catch {
    // continue
  }
  ...
  // If we have a current poster, put it first
  if (currentPoster?.image) {
    // Remove it from the list if present (to avoid duplicates)
    const filtered = allPosters.filter((url) => url !== currentPoster.image);
    // Current poster first, then the rest sorted by score
    return {
      posters: [currentPoster.image, ...sortAndLimit(filtered)],
      source: currentPoster.source,
      sourcesTried,
    };
  }

  // No current poster, return all sorted
  const sortedPosters = sortAndLimit(allPosters);
  return {
    posters: sortedPosters,
    source: sourcesTried[sourcesTried.length - 1] || null,
    sourcesTried,
  };
}
