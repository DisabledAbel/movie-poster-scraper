...
export async function fetchTmdbPosterCandidates(movie, year) {
  ...
  try {
    ...
    const results = (Array.isArray(payload?.results) ? payload.results : [])
      .filter((item) => item && isTitleMatch(movie, item.title || item.original_title));
...
export async function fetchImdbPosterCandidates(movie, year) {
  ...
  // Filter to only matching titles
  const prioritized = filteredItems
    .filter((item) => item && isTitleMatch(movie, item.l))
    .slice(0, 15);
...
export async function fetchItunesPosterCandidates(movie, year) {
  ...
  const payload = await response.json();
  const results = (Array.isArray(payload?.results) ? payload.results : [])
    .filter((item) => item && isTitleMatch(movie, item.trackName));
