...
/**
 * Fetches the lead character (first billed cast member) from TMDB
 * Returns object with character name and actor name
 */
export async function fetchTmdbLeadCharacter(movie, year) {
  const apiKey = process.env.TMDB_API_KEY;
  const trimmedMovie = typeof movie === "string" ? movie.trim() : "";
  if (!trimmedMovie || !apiKey) return null;
  ...
  try {
    ...
    const creditsUrl = `https://api.themoviedb.org/3/movie/${bestMatch.id}/credits?api_key=${apiKey}`;
    const creditsResponse = await fetch(creditsUrl);
    if (!creditsResponse.ok) return null;

    const creditsPayload = await creditsResponse.json().catch(() => null);
    const cast = Array.isArray(creditsPayload?.cast) ? creditsPayload.cast : [];
    
    if (!cast.length) return null;

    // Get the first cast member as lead character
    const lead = cast[0];
    return {
      character: lead?.character || null,
      actor: lead?.name || null,
    };
  } catch {
    return null;
  }
...
/**
 * Fetches lead character from IMDB (uses suggestion API - no key needed)
 */
export async function fetchImdbLeadCharacter(movie, year) {
  const trimmedMovie = typeof movie === "string" ? movie.trim() : "";
  ...
  try {
    ...
    const exactMatch = filteredItems.find((item) => item?.l?.toLowerCase() === trimmedMovie.toLowerCase());
    const bestItem = exactMatch || filteredItems[0];
    
    // IMDB suggestion API doesn't have cast info, so we just get the actor from the poster's "i" field if available
    const imageItem = bestItem?.i?.[0];
    if (imageItem) {
      return {
        character: null,
        actor: imageItem?.name || imageItem || null,
      };
    }
    
    // No cast info available in this API, but we can show year/length info at least
    return {
      year: bestItem?.y || null,
      type: bestItem?.q?.md || null,
      actor: null,
      character: null,
    };
  } catch {
