...
export async function fetchImdbPosterCandidates(movie, year) {
  ...
  const exactMatches = (payload?.d || [])
    .filter((item) => item?.id?.startsWith("tt"))
    .filter((item) => item?.l?.toLowerCase() === normalizedSearch);

  const yearMatches = (payload?.d || [])
    .filter((item) => item?.id?.startsWith("tt"))
    .filter((item) => year === null || item?.y === year);

  const prioritized = exactMatches.length
    ? [...exactMatches, ...yearMatches.filter((m) => !exactMatches.includes(m))]
    : yearMatches.length
      ? yearMatches
      : (payload?.d || []).filter((item) => item?.id?.startsWith("tt"));
