...
export function isTitleMatch(searchTitle, candidateTitle) {
  if (typeof searchTitle !== "string" || typeof candidateTitle !== "string") return false;

  const normalize = (s) => s
    .toLowerCase()
    .replace(/^(the|a|an)\s+/i, "")
    .replace(/\s+\((film|movie|tv series|video|short|\d{4})\)$/i, "")
    .replace(/[^a-z0-9]/g, "")
    .trim();

  const s1 = normalize(searchTitle);
  const s2 = normalize(candidateTitle);
