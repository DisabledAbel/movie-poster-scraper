...
export function movieRelevance(title, year, requestedTitle, requestedYear) {
  const exactTitle = normalizeMovieTitle(title) === normalizeMovieTitle(requestedTitle);
  if (!exactTitle) return 0;
  if (requestedYear === null) return 2;

  const candidateYear = Number.parseInt(String(year ?? ""), 10);
  return candidateYear === requestedYear ? 3 : 2;
}

export function rankPosterCandidates(candidates, max = 15) {
  const normalized = candidates
    .map((candidate, index) => typeof candidate === "string"
      ? { url: candidate, relevance: 0, index }
      : { ...candidate, index })
    .filter((candidate) => looksLikeCleanImageUrl(candidate.url));

  const bestByUrl = new Map();
  for (const candidate of normalized) {
    const key = canonicalPosterKey(candidate.url);
    const existing = bestByUrl.get(key);
    if (!existing
      || (candidate.relevance ?? 0) > (existing.relevance ?? 0)
      || ((candidate.relevance ?? 0) === (existing.relevance ?? 0)
        && posterScore(candidate.url) > posterScore(existing.url))) {
  ...
  return [...bestByUrl.values()]
    .sort((a, b) =>
      ...
      || posterScore(b.url) - posterScore(a.url)
      || a.index - b.index)
    .slice(0, max);
}
