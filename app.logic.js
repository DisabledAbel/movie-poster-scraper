...
export function initMoviePosterApp() {
  ...
  function rankAndLimitLocalSuggestions(query) {
    ...
    const startsWith = LOCAL_MOVIE_SUGGESTIONS.filter((movieTitle) => movieTitle.toLowerCase().startsWith(q));
    const contains = LOCAL_MOVIE_SUGGESTIONS.filter((movieTitle) => !movieTitle.toLowerCase().startsWith(q) && movieTitle.toLowerCase().includes(q));
  ...
  function renderSuggestions(suggestions) {
    ...
    suggestionsEl.innerHTML = "";
    suggestions.forEach((movieTitle, index) => {
      const li = document.createElement("li");
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "suggestion-btn";
      btn.textContent = movieTitle;
